...
// youtubeCRM_video_snapshots 에 스냅샷을 남긴다. 공용 테이블에 쓰는 유일한 V4 라우트.
const SYNC_LIMIT = 30
const CONCURRENCY = 5
...
export async function POST(request: Request) {
  try {
    ...
    const account = (accounts || []).find((a) => a.api_key && a.is_active !== false) || (accounts || [])[0]
    if (!account?.api_key) {
    ...
    const syncOne = async (video: (typeof targets)[number]) => {
      ...
      try {
        ...
        const { error: snapshotError } = await supabaseAdmin.from(V4_TABLES.videoSnapshots).insert({
          video_id: video.id,
        ...
        if (snapshotError) {
          ...
          console.error('V4 sync-stats snapshot insert 실패', snapshotError)
        }
        updated += 1
      } catch (e) {
        console.error('V4 sync-stats 영상 갱신 실패', video.id, e)
        failures.push(video.id)
      }
    }
